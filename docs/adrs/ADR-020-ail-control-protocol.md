...
Claude Code                    Gateway
    │                            │
    ├─ agentcards:execute() ────►│ Execute Layer 0
    │                            │
    │◄── {status: "layer_complete", results: [...]} ──┤
    │                            │
    │   [Agent analyzes results] │
    │                            │
    ├─ agentcards:continue() ───►│ Execute Layer 1
    │                            │
    │◄── {status: "layer_complete", results: [...]} ──┤
    │                            │
    │   [Agent finds XML files]  │
    │                            │
    ├─ agentcards:replan() ─────►│ GraphRAG adds XML parser
    │                            │
    │◄── {status: "layer_complete", new_tasks: [...]} │
    │                            │
    └─ agentcards:continue() ───►│ Complete
```
...
const response = await agentcards.execute({
