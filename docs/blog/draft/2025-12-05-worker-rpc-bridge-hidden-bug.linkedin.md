...
Why did tests pass? They used mock data, not real functions. No integration test actually called the
tools.

The fix wasn't a one-liner. We rebuilt the architecture:

→ Pass tool _definitions_ (serializable) instead of functions → Generate proxies in an isolated
Worker → Route calls through an RPC bridge → Trace everything natively (no stdout parsing hacks)
