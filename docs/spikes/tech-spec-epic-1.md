...
- Migration tool (agentcards init) lisant mcp.json de Claude Code
...
- Filesystem access préservé (pas Docker, accès direct ~/.agentcards/)
...
```bash
# Initialize AgentCards and migrate MCP configuration
...
export interface DatabaseAPI {
   ...
   * @param dbPath - Database file path (default: ~/.agentcards/agentcards.db)
   */
...
export interface ConfigAPI {
   ...
   * @param path - Config file path (default: ~/.agentcards/config.yaml)
...
2. Parse existing mcp.json → extract MCP server configs
    |
    v
3. Generate ~/.agentcards/config.yaml with migrated servers
    |
    v
4. Initialize PGlite database (~/.agentcards/agentcards.db)
...
8. Display migration summary:
    ...
    - Config path: ~/.agentcards/config.yaml
    |
    v
9. Print instructions for updating mcp.json to point to AgentCards
```
...
3. Log destination:
    ...
    - File: ~/.agentcards/logs/agentcards.log (all levels, JSON format)
    |
    v
4. Metrics storage:
...
- **Database Initialization (Story 1.2):**
  ...
  try {
    ...
    if (error.code === "EACCES") {
      throw new DBInitError("Permission denied. Check ~/.agentcards/ permissions.");
...
- **Database Recovery (Story 1.2):**
  - Automatic backup: Create `agentcards.db.backup` on first successful init
  - Manual recovery: `agentcards init --restore-backup` command
...
- **File Output (ALL levels):**
  - Location: `~/.agentcards/logs/agentcards.log`
