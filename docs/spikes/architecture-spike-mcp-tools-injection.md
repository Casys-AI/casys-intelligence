...
**Concept:** Fournir un module `agentcards` virtuel via import map ou dynamic import interception.
...
// Sandbox side
import { searchTools } from "agentcards";
...
**Module:** `agentcards` (virtual import)
...
1. **Create API Bridge Module** (4h)
   - `src/sandbox/agentcards-bridge.ts`
...
3. **Sandbox Executor Integration** (3h)
   ...
   - Inject agentcards-bridge into worker context
...
1. **Gateway Integration** (3h)
   - Add `agentcards:execute_code` tool to gateway
...
**Question:** Comment fournir le module `agentcards` au sandbox?
...
const preamble = `
import * as agentcards from "data:text/javascript,${encodedBridge}";
globalThis.agentcards = agentcards;
`;
```
