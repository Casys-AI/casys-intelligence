...
**Epic:** Future (Epic 5 - Ecosystem Integration)
...
3. Unified experience: Smithery (discovery/auth) + AgentCards (intelligence/optimization)
...
```
Smithery Registry → AgentCards PGlite → Discovery
```
...
```
AgentCards → Smithery Gateway → MCP Servers
```
- Smithery handles all auth
- AgentCards adds intelligence layer
...
Intelligence: AgentCards (context optimization + DAG)
```
...
**What does AgentCards add on top?**
...
- [ ] Create minimal TypeScript POC:
  ...
  // 4. Proxy through AgentCards gateway
...
- ❓ **Can AgentCards act as OAuth client for Smithery?**
- ❓ **Or does user need separate Smithery account + AgentCards setup?**
...
| **Smithery HTTP** | Config keys on Smithery, OAuth in AgentCards | Medium-High | Smithery API |
...
1. Can AgentCards implement OAuth 2.1 client for Smithery?
2. Is HTTP transport worth the complexity vs. stdio?
3. Does this provide real value to users?

**Next Steps if Go:**
- Implement OAuth 2.1 + PKCE flow in AgentCards
...
**What We Don't Know:**
- ❓ Can AgentCards act as Smithery OAuth client?
...
- ✅ **Discovery UX** - Browse/search servers in AgentCards UI
- ✅ **Differentiation** - Smithery (hosting) + AgentCards (intelligence)
...
With Smithery:
1. Browse servers in AgentCards
...
3. **What's the actual user demand?**
   - Survey: Ask AgentCards users if they want Smithery integration
...
- [AgentCards Research - Market Analysis](../research/research-market-2025-11-11.md#compétiteur-1--smithery-community-platform)
- [AgentCards PRD - Competitive Analysis](../PRD.md)
...
**Time Spent:** 1h
**Status:** API exploration successful
...
**Total Time Budget:** 6 hours max
**Time Spent:** 1h (Phase 1 only)
**Remaining:** 5h (not used - spike paused)
