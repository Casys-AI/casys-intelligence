...
const exportGraphData = (format: "json" | "graphml") => {
  ...
  if (format === "json") {
    ...
    downloadBlob(blob, `agentcards-graph-${new Date().toISOString()}.json`);
  } else {
    // GraphML format
    const graphml = generateGraphML(cyRef.current);
    const blob = new Blob([graphml], { type: "application/xml" });
    downloadBlob(blob, `agentcards-graph-${new Date().toISOString()}.graphml`);
  }
};
