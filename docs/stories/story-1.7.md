...
to AgentCards automatically, **So that** I don't have to manually reconfigure everything.
...
- [x] Generate AgentCards config.yaml with migrated servers
...
- docs/stories/1-7-migration-tool-agentcards-init.context.xml
...
- ✅ CLI command `agentcards init` working with Cliffy framework
...
- ✅ Generation of AgentCards config.yaml
...
des configurations MCP de Claude Desktop vers AgentCards. L'implémentation est exceptionnelle avec:
...
| ------- | -------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AC1** | CLI command `agentcards init` implemented                      | ✅ **IMPLEMENTED** | [src/cli/commands/init.ts:20-46](src/cli/commands/init.ts#L20-L46) createInitCommand() avec Cliffy<br/>[src/main.ts:67](src/main.ts#L67) Command registered `.command("init", createInitCommand())`<br/>**Test:** Integration test "CLI command registration" ✅                                                                               |
...
| **AC3** | Parsing mcp.json + extraction MCP servers                      | ✅ **IMPLEMENTED** | [src/cli/config-migrator.ts:83-84](src/cli/config-migrator.ts#L83-L84) Uses MCPServerDiscovery.loadConfig()<br/>Story context confirms normalizeConfig() converts Claude format → AgentCards format<br/>**Test:** ConfigMigrator tests with sample config ✅                                                                                   |
| **AC4** | Generation ~/.agentcards/config.yaml                           | ✅ **IMPLEMENTED** | [src/cli/config-migrator.ts:100-125](src/cli/config-migrator.ts#L100-L125) Complete workflow:<br/>• Creates config dir (line 105)<br/>• Writes YAML (line 123)<br/>• Cleans undefined values (lines 109-121)<br/>**Test:** Preview test shows correct path ✅                                                                                  |
...
| **AC7** | Template nouveau config mcp.json (gateway)                     | ✅ **IMPLEMENTED** | [src/cli/config-migrator.ts:242-260](src/cli/config-migrator.ts#L242-L260) displayNewMCPConfig():<br/>• Lines 246-253: Template JSON with agentcards command<br/>• Line 257: "💡 AgentCards now acts as gateway..."<br/>• Format: `{ mcpServers: { agentcards: { command: "agentcards", args: ["serve"] } } }`                                 |
...
| Generate AgentCards config.yaml                              | ✅ Complete | ✅ **VERIFIED** | [src/cli/config-migrator.ts:100-125](src/cli/config-migrator.ts#L100-L125) Writes YAML with cleaned config                                                                                                        |
...
- ✅ Config location: ~/.agentcards/config.yaml (architecture.md:125)
- ✅ Database location: ~/.agentcards/.agentcards.db (architecture.md:126)
