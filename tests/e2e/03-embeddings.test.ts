...
Deno.test("E2E 03: Embedding generation with BGE-M3", async (t) => {
  ...
  try {
    ...
    await t.step("5. Test embedding similarity search", async () => {
      ...
      console.log(`  Top match: ${topResult.tool_name} (similarity: ${topResult.similarity.toFixed(3)})`);
    });
    ...
    await t.step("8. Test embedding accuracy with known semantic relationships", async () => {
      ...
      for (const testCase of testCases) {
        ...
        console.log(
          `  ${testCase.description}: similarity = ${cosineSimilarity.toFixed(3)} (expected ${testCase.inverse ? '<' : '>'} ${testCase.expectedSimilarity})`,
        );
        ...
        if (testCase.inverse) {
          ...
          assert(
            ...
            `${testCase.description}: similarity ${cosineSimilarity.toFixed(3)} should be < ${testCase.expectedSimilarity}`,
          );
        } else {
