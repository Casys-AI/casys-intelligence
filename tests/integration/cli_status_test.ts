 ...
 *
 * Tests AC6: agentcards status CLI command
 */

import { assert, assertEquals } from "@std/assert";

Deno.test({
  name: "AC6: agentcards status command is registered and shows help",
...
Deno.test({
  name: "AC6: agentcards status fails gracefully when no config found",
  sanitizeResources: false,
  async fn() {
